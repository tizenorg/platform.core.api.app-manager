...
static int __update_app_event_info(app_event_info **head, int req_id,
				app_event_type_e event_type,
				app_event_state_e event_state)
{
	app_event_type_e evt_type;
	app_event_state_e evt_state;
	app_event_info *tmp;

	if (__find_app_event_info(head, req_id, &evt_type, &evt_state) != 0) {
		__add_app_event_info(head, req_id, event_type, event_state);
		return APP_MANAGER_ERROR_REQUEST_FAILED;
	}
				...
				const char *val, const void *pmsg, void *data)
{
	...
	if (strcasecmp(key, "start") == 0) {
		...
		if (__find_app_event_info(&(app_event->head), req_id, &event_type,
				&event_state) == 0) {
			__update_app_event_info((&app_event->head), req_id, event_type,
