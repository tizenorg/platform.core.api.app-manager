...
static int __add_app_event_info(app_event_info **head, int req_id,
				...
				app_event_state_e event_state)
{
	_app_event_info *event_info;
	_app_event_info *current;
	_app_event_info *prev;

	event_info = (_app_event_info *)calloc(1, sizeof(_app_event_info));
	if (event_info == NULL) {
		LOGE("calloc failed");
		return -1;
	}
				...
				const char *val, const void *pmsg, void *data)
{
	...
	if (strcasecmp(key, "start") == 0) {
		...
		//find info at app_manager_event_s

		//update app event info
