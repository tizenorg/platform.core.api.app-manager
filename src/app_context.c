...
static int app_context_foreach_all_app_context_cb(const aul_app_info *aul_app_context, void *cb_data)
{
	...
	if (foreach_context->iteration == true) {
		...
		if (cnt > 0) {
			for (i = 0; i <cnt; i++) {
				aul_app_get_appid_bypid(aul_app_context->pid, app_id, sizeof(app_id));
				if (app_context_create(aul_app_context->appid, aul_app_context->pid, true, &app_context) == APP_MANAGER_ERROR_NONE) {
					foreach_context->iteration = foreach_context->callback(app_context, foreach_context->user_data);
					app_context_destroy(app_context);
				}
			}
...
static int app_context_create(const char *app_id, pid_t pid, bool is_sub_app, app_context_h *app_context)
{
	...
	if (aul_app_get_appid_bypid(app_context_created->pid, app_context_created->pkg_id, 256/*temporarily*/) < 0)
		return app_manager_error(APP_MANAGER_ERROR_IO_ERROR, __FUNCTION__, NULL);
