...
static int __find_app_event_info(app_event_info **head, int req_id, app_event_type_e *event_type)
{
	app_event_info *tmp;
...
static int __add_app_event_info(app_event_info **head, int req_id, app_event_type_e event_type)
{
...
static int _convert_status_type(int status_type)
{
	...
	if ((status_type & APP_EVENT_STATUS_TYPE_ENABLE) == APP_EVENT_STATUS_TYPE_ENABLE)
		result += PKGMGR_CLIENT_STATUS_ENABLE_APP;

	if ((status_type & APP_EVENT_STATUS_TYPE_DISABLE) == APP_EVENT_STATUS_TYPE_DISABLE)
...
API int app_event_set_event_status(app_event_h handle, int status_type)
{
	...
	if (ret != PKGMGR_R_OK) {
		LOGE("failed to set status[%d], ret[%d]", status_type, ret);
		return APP_MANAGER_ERROR_REQUEST_FAILED;
	}
		...
		void *user_data)
{
	...
	ret = pkgmgr_client_listen_app_status(handle->pc, _app_event_handler, handle);
	if (ret < PKGMGR_R_OK) {
		LOGE("failed to set event cb");
		return APP_MANAGER_ERROR_REQUEST_FAILED;
	}
...
API int app_event_destroy(app_event_h handle)
{
	...
	if (ret != 0) {
		LOGE("failed to free pkgmgr client");
		return APP_MANAGER_ERROR_REQUEST_FAILED;
	}
